...
class TheMovieViewModel : ViewModel() {
    ...
    private val _favoriteMovies = mutableStateListOf<FavoriteMovie>()
    ...
    fun addFavoriteMovie(movie: Result) {
        // Check if the movie is already favorited to prevent duplicates
        if (_favoriteMovies.none { it.id == movie.id }) {
            _favoriteMovies.add(
                FavoriteMovie(
                    id = movie.id,
                    title = movie.title,
